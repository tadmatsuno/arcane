...
def find_moogsilent():
    ...
    if not "moog_silent" in arcane_config.keys():
        print("MOOGSILENT is not set in the .arcanesrc file")
        print("Call set_moogsilent_path to set the path")
        return
    moog_path  = arcane_config["moog_silent"]
    if not os.path.exists(moog_path):
        print("MOOGSILENT does not exist at the specified path:{0:s}".format(moog_path))
        print("Call set_moogsilent_path to set the path")
        return
    print("MOOGSILENT location: {0:s}".format(moog_path))
    return

def set_moogsilent_path(moog_path):
    if os.path.exists(moog_path):
...
def write_linelist(linelist,flinelist,isabfind=False,default_gamma_vw=3.,dwvl_margin=2.0,head1=True):
    if isinstance(linelist,(dict,pandas.DataFrame,Linelist)):
        if isinstance(linelist,Linelist) and hasattr(linelist,'scaled') and not linelist.scaled:
            ...
            linelist.get_scaling()
            linelist.apply_scaling()
...
def run_moog(mode, linelist, run_id = '', workdir = '.',
    ...
    **kw_args):
    ...
    1. to specify abundance of an element, provide A_{proton_number} = log(N_X/N_H) + 12 or AX_dict
        e.g., AX_dict = {6:8.43, 8: 8.43} or A_6 = 8.43
    2. to specify isotope ratio, provide Isotope_dict or I_{molecule id}_{isotope_id}
        e.g., Isotope_dict = {"106.00113": 0.01} or I_106_00113 = 0.01
        Note that for consistency, the given isotope ratio will be multiplied to the original abundance,
        which is the opposite to what is adopted in MOOG. In parctice, 1/value is given to MOOG, thus 
        VALUE CANNOT BE 0.0.
...
#        feh = 
    ...
    if "AX_dict" in kw_args.keys():
        for atomnum in kw_args["AX_dict"].keys():
            if solar_moog[atomnum-1] == -5.0:
                warnings.warn(f'By default, MOOG does not support atom with Proton number {atomnum}.\n'
                              'This may cause an issue in the calculation.\n')
            abundances[atomnum] = kw_args["AX_dict"][atomnum] - (solar_moog[atomnum-1] + feh)
    if "Isotope_dict" in kw_args.keys():
        for isospecies in kw_args["Isotope_dict"].keys():
            ...
            value = kw_args["Isotope_dict"][isospecies]
            if value == 0.0:
                warnings.warn(f'Isotope {isospecies} is set to 0.0. This will cause an error in MOOG.\n'+\
                    'It will be replaced to 1e-4')
                value = 1e-4
            isotopes[isospecies] = 1.0/value
            
    for key,val in kw_args.items():
        if key.startswith('A_'):
            atomnum = int(key[2:])
            # Check if MOOG has a valid abundance for the atom
            if solar_moog[atomnum-1] == -5.0:
                warnings.warn(f'By default, MOOG does not support atom with Proton number {atomnum}.\n'
                              'This may cause an issue in the calculation.\n')
            # Check if the abundance is already in the dictionary
            if atomnum in abundances.keys():
                warnings.warn(f'Abundance for atom {atomnum} is already specified. Overwriting it.\n'+\
                    "A_X entries are prioritized over AX_dict inputs\n")
            # Set abundnace input for MOOG
            abundances[atomnum] = val - (solar_moog[atomnum-1] + feh)
        if key.startswith('I_'):
            isospecies = '.'.join(key[2:].split('_'))
            # Check if the isotope is already in the dictionary
            if isospecies in isotopes.keys():
                warnings.warn(f'Isotope {isospecies} is already specified. Overwriting it.\n'+\
                    "I_XXX_XXX entries are prioritized over Isotope_dict inputs\n")
            if val == 0.0:
                warnings.warn(f'Isotope {isospecies} is set to 0.0. This will cause an error in MOOG.\n'+\
                    'It will be replaced to 1e-4')
                val = 1e-4
            # Set isotopic scaling for MOOG
